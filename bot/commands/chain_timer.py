...
def register(client: discord.Client, tree: app_commands.CommandTree):
    ...
    @tree.command(name="chainstart", description="Start watching the faction chain timer (leadership only).")
    async def chainstart(interaction: discord.Interaction):
    ...
    @tree.command(name="chainstop", description="Stop watching the faction chain timer (leadership only).")
    async def chainstop(interaction: discord.Interaction):
        if not interaction.guild or not isinstance(interaction.user, discord.Member):
            return await interaction.response.send_message("Guild-only command.", ephemeral=True)
    ...
    @tree.command(name="pingme", description="Opt-in to chain saver pings even if you're offline.")
    async def pingme(interaction: discord.Interaction):
        if not interaction.guild:
            return await interaction.response.send_message("Faction-only command.", ephemeral=True)

        db.chain_optin_add(client.db_conn, interaction.guild.id, interaction.user.id)
        await interaction.response.send_message(
            "✅ You will be pinged when the chain timer is low.\n"
            "ℹ️ This opt-in resets when leadership runs `/chainstop`.",
            ephemeral=True,
        )

    @tree.command(name="noping", description="Opt-out of chain pings.")
    ...
    @tree.command(name="chainstatus", description="Show chain watcher status + current chain timeout.")
    async def chainstatus(interaction: discord.Interaction):
        if not interaction.guild:
            return await interaction.response.send_message("Guild-only command.", ephemeral=True)
        ...
        try:
            ...
            if not chain:
                ...
                chain_line = f"Chain: **Active** — id `{chain.get('id')}`, timeout **{chain.get('timeout')}s**"
